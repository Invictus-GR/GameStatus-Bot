...
ds = replaceOnce(
  ...
  "  return {\n    isOnline,\n    players,\n    maxPlayers,\n    queue: 0,\n    serverName: typeof attributes.name === 'string' && attributes.name.trim() ? attributes.name.trim() : null,\n    __dataSource: 'BattleMetrics'\n  };",
...
ds = replaceOnce(
  ...
  "  return {\n    isOnline,\n    players,\n    maxPlayers,\n    queue,\n    serverName: typeof server.name === 'string' && server.name.trim() ? server.name.trim() : null\n  };",
  'ReforgerMods server name'
);
...
ds = replaceOnce(
  ...
  'discovery accepts observed name'
);
...
ds = replaceOnce(
  ...
  'client exports identity methods'
);
...
bot = replaceOnce(
  ...
  try {
    await pool.query(\`
      INSERT INTO server_identity_state (id, server_name, reforgermods_server_id, updated_at)
      VALUES (1, $1, $2, NOW())
      ON CONFLICT (id) DO UPDATE SET
        server_name = EXCLUDED.server_name,
        reforgermods_server_id = EXCLUDED.reforgermods_server_id,
        updated_at = NOW();
    \`, [serverName, reforgerModsServerId]);
    return true;
  } catch (error) {
    console.error('❌ Failed to persist server identity:', error);
    return false;
  }
}
...
async function restoreServerIdentity() {
  try {
    const result = await pool.query(\`
      SELECT server_name, reforgermods_server_id
      FROM server_identity_state
      WHERE id = 1
    \`);
    ...
    reforgerModsClient.setIdentity({
      serverName: row.server_name,
      serverId: row.reforgermods_server_id
    });
    console.log(\`Restored server identity: ${'${currentServerName}'} / ${'${row.reforgermods_server_id}'}\`);
    return true;
  } catch (error) {
    console.error('❌ Failed to restore server identity:', error);
    return false;
  }
}
...
async function syncObservedServerIdentity(observedName, source) {
  ...
  try {
    ...
    console.warn(
      \`Server rename observed from ${'${source}'} but ReforgerMods identity sync failed:\`,
      error?.message || error
    );
    return false;
  }
...
function extractArmaHQServerName(html) {
  ...
  if (h1?.[1]) candidates.push(h1[1].replace(/<[^>]+>/g, ' '));

  const title = html.match(/<title[^>]*>([\\s\\S]*?)<\\/title>/i);
  if (title?.[1]) candidates.push(title[1]);

  for (const candidate of candidates) {
    const cleaned = decodeBasicHtmlEntities(candidate)
      .replace(/\\s+/g, ' ')
      .replace(/\\s*[|–—-]\\s*ArmaHQ.*$/i, '')
      .trim();
...
async function requestArmaHQPage() {`,
  'identity persistence and ArmaHQ name extraction'
);
...
bot = replaceOnce(
  ...
  "      serverData = result.value;\n      currentStatusDataSource = result.source;\n      if (serverData?.serverName) {\n        await syncObservedServerIdentity(serverData.serverName, result.source);\n      }\n      if (result.source === 'ArmaHQ') {",
  'proactive identity sync on status'
);
...
test('BattleMetrics exposes observed server name for identity synchronization', () => {
  const normalized = normalizeBattleMetricsServer({ data: { attributes: {
    name: 'EU | TLC | NEW NAME', status: 'online', players: 5, maxPlayers: 128
  } } });
  ...
  const fetchImpl = async url => {
    ...
    if (String(url).includes('/servers?')) {
      return { ok: true, json: async () => ({
        ...
        data: [{ id: 'new-room-id', name: 'EU | TLC | NEW NAME' }]
      }) };
    }
    throw new Error('unexpected request');
  };

  const client = createReforgerModsClient({
    fetchImpl,
    serverName: 'EU | TLC | OLD NAME',
    serverId: 'old-room-id'
  });

  const discovered = await client.discoverServerId('EU | TLC | NEW NAME');
  assert.equal(discovered, 'new-room-id');
  assert.deepEqual(client.getIdentity(), {
    serverName: 'EU | TLC | NEW NAME',
    serverId: 'new-room-id'
  });
...
test('persisted identity can be restored into the ReforgerMods client', () => {
  ...
  assert.deepEqual(client.getIdentity(), {
    serverName: 'RESTORED NAME',
    serverId: 'restored-id'
  });
});
`;
